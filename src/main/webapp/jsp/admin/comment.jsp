    ...
    <div class="all-list">
        ...
        <div class="list-page">
            <div class="page">
                ...
                <a class="page-last" href="${pageContext.request.contextPath }/admin/commentlist?pc=${pb.pc+1}"><i class="icon icon-last"></i></a>
                </c:if>
                <c:if test="${pb.pc==pb.tp}">
                <a class="page-last"><i class="icon icon-last"></i></a>
                </c:if>
                <form action="${pageContext.request.contextPath }/admin/commentlist" method="post">
                    <span class="currentPage">&nbsp;第<input type="text" name="pc" value="${pb.pc }"/>页，共${pb.tp }页</span>
                    <input type="submit" value="submit" style="display: none;"/>
                </form>
                <c:if test="${pb.pc>1}">
                <a class="page-first" href="${pageContext.request.contextPath }/admin/commentlist?pc=${pb.pc-1}"><i class="icon icon-first"></i></a>
                </c:if>
                <c:if test="${pb.pc==1}">
        ...
        <table id="table" cellspacing="0">
            ...
            <tr class="tr-main">
            	<td class="td-0"><a class="td-0-a">评论</a></td>
                <td>${comment.username }</td>
                <td>${comment.content }</td>
                <td>${comment.title }</td>
                <td><fmt:formatDate value="${comment.date }" pattern="yyyy年M月d日 HH:mm:SS"/></td>
                <td><a onclick="return confirm('确定要删除该评论？')" href="${pageContext.request.contextPath}/admin/deletecomment?id=${comment.id}">删除</a></td>
            </tr>
            ...
            <tr class="children">
                ...
                <td><a onclick="return confirm('确定要删除该回复？')" href="${pageContext.request.contextPath}/admin/deletereply?id=${reply.id}">删除</a></td>
            </tr>
            </c:forEach>
            </c:forEach>
            <tr>
